...
|------------|--------------------------------|--------|
| iOS app    | Swift + SwiftUI + SwiftData    | 🟡 Scaffolded — study loop + real PDF/photo capture. Authenticates with **Supabase JWT** — email **and Google OAuth** sign-in, `Authorization: Bearer` (retired `X-Cram-Secret` dropped) — via the `supabase-swift` SDK. Ships with committed non-secret Supabase config, so it authenticates on **standalone** (non-Xcode) launches, not only under the debugger; pending a live end-to-end generate test against Railway (Mac) |
...
| Web        | Next.js + React + Tailwind     | 🟡 **v0.6 built** — the "study desk": Supabase login, subjects with an exam countdown, browse sources/cards/quizzes, upload material → generate decks, **take a quiz** (MC graded client-side, short-answer via the backend's Claude grader), **review cards** (spaced repetition via a parity-tested TypeScript port of the iOS SM-2 scheduler), and a progress overview. Talks to the live backend with a Supabase Bearer JWT (CORS-gated). See [`web/README.md`](web/README.md) |
