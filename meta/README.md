...
| [deployment.md](deployment.md) | Host = Railway Hobby; the M1 reverse proxy (nginx) is baked into the container in front of uvicorn so the body/flood cap holds portably; why Vercel/Render were rejected for the backend; deploy packaging in `backend/`. Cross-refs ADR 0009. |
